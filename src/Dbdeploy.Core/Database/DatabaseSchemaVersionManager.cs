    ...
    public class DatabaseSchemaVersionManager : IAppliedChangesProvider
    {
        ...
        public virtual IList<ChangeEntry> GetAppliedChanges()
        {
            ...
            try
            {
                ...
                string sql = string.Format(CultureInfo.InvariantCulture, "SELECT ChangeId, Folder, ScriptNumber, ScriptName, ScriptStatus, ScriptOutput FROM {0}", changeLogTableName);
...
SELECT ChangeId FROM {0} WHERE Folder = @1 and ScriptNumber = @2",
                    ...
                    using (var reader = queryExecuter.ExecuteQuery(sql, script.Folder, script.ScriptNumber, script.ScriptName, (int)status, output ?? string.Empty))
                    {
                        reader.Read();
                ...
                else
                {
                    ...
                    queryExecuter.Execute(sql, script.Folder, script.ScriptNumber, script.ScriptName, (int)status, output ?? string.Empty, script.ChangeId);
                }
            }
            catch (DbException e)
            {
        ...
        public void CreateChangeLogTable()
        {
            ...
            string script = syntax.CreateChangeLogTableSqlScript(changeLogTableName);
